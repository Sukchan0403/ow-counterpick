...
# 드래프트 시뮬레이션(사전 팀 평가) 모드에서 5명 각각의 percentage를 팀 종합
# 점수 하나로 합칠 때 쓰는 역할별 가중치. "탱커가 게임에 미치는 영향력이 가장
# 크고, 그다음 딜러, 그다음 힐러"라는 판단(요청자 피드백)을 반영한 초기
# 튜닝값 — 실전 데이터로 검증된 값은 아니며, 상대적 크기(탱커>딜러>힐러)만
# 의미가 있다. 가중 평균(weighted average)이라 절대 크기 자체는 결과에
# 영향 없다 — 예: {1.5,1.2,1.0}과 {3,2.4,2}는 같은 결과를 낸다.
ROLE_INFLUENCE_WEIGHT = {"tank": 1.5, "damage": 1.2, "support": 1.0}
